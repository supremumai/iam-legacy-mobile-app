      ...
      >
        ...
        <View style={{ backgroundColor: '#111008', borderBottomLeftRadius: 24, borderBottomRightRadius: 24, alignItems: 'center', paddingTop: 32, paddingBottom: 28 }}>
          <View style={{ width: 80, height: 80, borderRadius: 40, backgroundColor: '#1c1a14', borderWidth: 2, borderColor: 'rgba(201,168,76,0.22)', alignItems: 'center', justifyContent: 'center' }}>
            <Text style={{ fontFamily: Fonts.bodyBold, fontSize: 28, color: '#FFFFFF' }}>JL</Text>
          </View>
          <Text style={{ fontFamily: Fonts.heading, fontSize: 22, color: '#FFFFFF', marginTop: 12 }}>Julio Legacy</Text>
          <Text style={{ fontFamily: Fonts.body, fontSize: 14, color: 'rgba(255,255,255,0.55)', marginTop: 4 }}>Entrepreneur · Investor</Text>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4, marginTop: 4 }}>
            <Ionicons name="location-outline" size={13} color="rgba(255,255,255,0.55)" />
            <Text style={{ fontFamily: Fonts.body, fontSize: 13, color: 'rgba(255,255,255,0.55)' }}>Miami, FL</Text>
          </View>
          <View style={{ flexDirection: 'row', marginTop: 20, gap: 40 }}>
            {[{ value: '1.2k', label: 'Followers' }, { value: '348', label: 'Following' }, { value: '12', label: 'Courses' }].map((stat) => (
              <View key={stat.label} style={{ alignItems: 'center' }}>
                <Text style={{ fontFamily: Fonts.bodyBold, fontSize: 20, color: '#c9a84c' }}>{stat.value}</Text>
                <Text style={{ fontFamily: Fonts.body, fontSize: 12, color: 'rgba(255,255,255,0.55)', marginTop: 2 }}>{stat.label}</Text>
              </View>
            ))}
          </View>
        </View>
        ...
        <View style={{ flexDirection: 'row', paddingHorizontal: 20, marginTop: 20, gap: 12 }}>
          {QUICK_ACTIONS.map((action) => (
            ...
            >
              ...
              <Text style={{ fontFamily: Fonts.body, fontSize: 12, color: '#FFFFFF', marginTop: 4 }}>{action.label}</Text>
            </View>
          ))}
        </View>
            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginTop: 12, gap: 8 }}>
              {TAGS.map((tag) => (
                <View key={tag} style={{ backgroundColor: '#111008', borderRadius: 999, borderWidth: 1, borderColor: 'rgba(201,168,76,0.22)', paddingHorizontal: 12, paddingVertical: 4 }}>
                  <Text style={{ fontFamily: Fonts.body, fontSize: 13, color: '#c9a84c' }}>{tag}</Text>
                </View>
              ))}
            </View>
          </View>
        ...
        <View style={{ marginTop: 24 }}>
          <Text style={{ paddingHorizontal: 20, fontFamily: Fonts.heading, fontSize: 18, color: '#c9a84c' }}>My Courses</Text>
          <ScrollView
            horizontal
            ...
            {MY_COURSES.map((course) => (
              ...
              >
                ...
                <View style={{ padding: 14 }}>
                  <Text style={{ fontFamily: Fonts.bodyBold, fontSize: 14, color: '#FFFFFF' }}>{course.title}</Text>
                  ...
                  <Text style={{ fontFamily: Fonts.body, fontSize: 12, color: 'rgba(255,255,255,0.55)', marginTop: 6 }}>{course.percent}% complete</Text>
                </View>
              </View>
            ))}
          </ScrollView>
        ...
        {/* Zone 5: Settings */}
        <View style={{ paddingHorizontal: 20, marginTop: 24 }}>
          <Text style={{ fontFamily: Fonts.heading, fontSize: 18, color: '#c9a84c' }}>Settings</Text>
              ...
              <View key={item.label}>
                <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, paddingVertical: 16 }}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
                    ...
                    <Text style={{ fontFamily: Fonts.body, fontSize: 15, color: item.danger ? '#EF4444' : '#FFFFFF' }}>{item.label}</Text>
                  </View>
                  <Ionicons name="chevron-forward-outline" size={16} color="rgba(255,255,255,0.55)" />
                </View>
